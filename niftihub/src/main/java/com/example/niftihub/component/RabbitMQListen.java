...
public class RabbitMQListen {
    ...
    public void handingSystemMessage(WebsocketMessage websocketMessage) throws IOException {
        //判断系统消息类型，进行相应操作
        ...
        if(systemMessageType == SystemMessageType.READ_MESSAGE){
            ...
            Set<Object> unreadMessageSet =
                    messageService.getUnreadMessage(websocketMessage.getFromUid());
            for(Object object:unreadMessageSet){
                //查询mysql获得消息
                MessageDO data = messageService.selectMessage(object.toString());
                JSONObject unreadMessage = JSONUtil.createObj()
                        .putOnce("type",SystemMessageType.UNREAD_MESSAGE)
                        .putOpt("data",data);
                JSONObject returnSystemMessage = JSONUtil.createObj()
                        .putOnce("system",true)
                        .putOpt("message",unreadMessage)
                        .putOpt("time",DateUtil.now());
                ChatEndpoint.sendMessage(returnSystemMessage, websocketMessage.getFromUid());
            }
        }else {
            log.error("错误的系统消息");
        }
